...
PointCloud → RRT* → B-Spline → L-BFGS → Final Trajectory
...
def plot_comparison(data, output_dir='.'):
    ...
    def save_grouped_bar(metric_key, ylabel, title, filename):
        ...
        ax.bar(x - width / 2, ct, width, label='Classical RRT*', color=C_COLOR)
        ax.bar(x + width / 2, et, width, label='Enhanced RRT*', color=E_COLOR)
    ...
    save_grouped_bar('rrt_time', 'Time (ms)', 'RRT* Search Time', 'fig1_pipeline_timing.png')
    ...
    save_grouped_bar('rrt_path_len', 'Path Length (m)', 'RRT* Raw Path Length', 'fig4_rrt_path_length.png')
    ...
    save_grouped_bar('rrt_clearance', 'Clearance (m)', 'RRT* Min Clearance', 'fig7_rrt_clearance.png')
    ...
    save_grouped_bar('lbfgs_cost_reduction', 'Cost Reduction (%)', 'L-BFGS Cost Reduction', 'fig12_lbfgs_cost_reduction.png')

    # ── Stage breakdown: split into two separate single-chart images ──
    stages = ['RRT* Search', 'B-Spline Conv', 'L-BFGS Opt']
    c_stage_avg = [0, 0, 0]
    e_stage_avg = [0, 0, 0]
    for s in scenarios:
    ...
    for i, (stage, color) in enumerate(zip(stages, colors_stage)):
        ...
        bars = ax.bar(['Classical RRT*', 'Enhanced RRT*'], vals, width=0.5,
    ...
    ax.set_ylabel('Time (ms)')
    ax.set_title('Enhanced RRT*: Per-Scenario Pipeline Breakdown')
    ...
    imp_pairs = [
        ('RRT* Search Time', 'rrt_time', True),
        ('RRT* Path Length', 'rrt_path_len', True),
        ('RRT* Tree Nodes', 'rrt_nodes', True),
        ('RRT* Clearance', 'rrt_clearance', False),
        ('L-BFGS Opt Time', 'lbfgs_time', True),
        ('L-BFGS Iterations', 'lbfgs_iters', True),
        ('L-BFGS Cost Reduc.', 'lbfgs_cost_reduction', False),
        ('Trajectory Length', 'traj_length', True),
        ('Trajectory Clearance', 'traj_clearance', False),
        ('Trajectory Smoothness', 'traj_smoothness', True),
        ('Max Velocity', 'traj_max_vel', True),
        ('Max Acceleration', 'traj_max_acc', True),
        ('End-to-End Time', 'total_time', True),
    ...
    ax.set_title('Average Improvement of Enhanced RRT* over Classical RRT*\n(across all pipeline stages and scenarios)')
    ...
    # ── Figure 6: Path Length Evolution (RRT → Optimized) ────────────
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color=C_COLOR, label='Classical RRT* → Optimized', markersize=8, linewidth=2),
        Line2D([0], [0], marker='s', color=E_COLOR, label='Enhanced RRT* → Optimized', markersize=8, linewidth=2, linestyle='--'),
    ]
    ax.legend(handles=legend_elements, fontsize=9, loc='upper left')
    ax.set_xticks([xi + 0.5 for xi in x2])
    ax.set_xticklabels(short_names, rotation=0, ha='center', fontsize=8)
    ax.set_ylabel('Path Length (m)')
    ax.set_title('Path Length Evolution: Raw RRT* → B-Spline + L-BFGS Optimized')
    ax.grid(axis='y', alpha=0.3)
    ...
    ax.bar(x - width/2, c_sr, width, label='Classical RRT*', color=C_COLOR)
    ax.bar(x + width/2, e_sr, width, label='Enhanced RRT*', color=E_COLOR)
...
def plot_route_showcase(search_dir='.', output_dir='.', defense_style=False):
    ...
    stage_name = {'rrt': 'RRT* raw', 'optimized': 'Optimized'}
...
if __name__ == '__main__':
    ...
    output_dir = sys.argv[2] if len(sys.argv) > 2 else 'loads'
