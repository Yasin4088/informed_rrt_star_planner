...
namespace ego_planner
{
...
#ifdef USE_INFORMED_RRT_STAR
    bspline_optimizer_->informed_rrt_star_.reset(new InformedRRTstar);
    bspline_optimizer_->informed_rrt_star_->initGridMap(grid_map_, Eigen::Vector3i(100, 100, 100));
#else
    // DISABLED: Original A* initialization (see dyn_a_star.h)
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
      ...
      else // Initial path generated from previous trajectory.
      {
            ...
            pseudo_arc_length.size() != segment_point.size())
        {
          ROS_WARN_THROTTLE(2.0,
                            "[PLAN] Degenerate trajectory resample (pseudo=%zu seg_pts=%zu); "
                            "forcing polynomial re-init to avoid invalid sampling / malloc failure.",
                            pseudo_arc_length.size(), segment_point.size());
        ...
        do
        {
          ...
          sample_length = 0;
          id = 0;
          constexpr int kInnerIterCap = 400000;
          int inner_iters = 0;
          while ((id + 1) < pseudo_arc_length.size() && (id + 1) < segment_point.size() &&
                 sample_length <= pseudo_arc_length.back() + 1e-9 && inner_iters < kInnerIterCap)
          {
            ++inner_iters;
            const double denom = pseudo_arc_length[id + 1] - pseudo_arc_length[id];
            if (sample_length >= pseudo_arc_length[id] && denom > 1e-12 &&
                sample_length < pseudo_arc_length[id + 1])
            {
              point_set.push_back((sample_length - pseudo_arc_length[id]) / denom * segment_point[id + 1] +
                                  (pseudo_arc_length[id + 1] - sample_length) / denom * segment_point[id]);
          ...
          if (inner_iters >= kInnerIterCap)
          {
            ROS_WARN_THROTTLE(3.0, "[PLAN] Arc resample iteration cap hit; forcing polynomial re-init.");
            flag_force_polynomial = true;
            flag_regenerate = true;
            break;
          }
          point_set.push_back(local_target_pt);
        } while (!flag_regenerate && point_set.size() < 7);
    ...
    if (segments.empty() && bspline_optimizer_->needFreshReplan())
    {
      ROS_WARN("[PLAN] Initial trajectory deeply overlaps obstacles; rebuilding initialization with Informed RRT*.");
#ifdef USE_INFORMED_RRT_STAR
      if (bspline_optimizer_->informed_rrt_star_ &&
          bspline_optimizer_->informed_rrt_star_->InformedRRTstarSearch(std::max(0.10, pp_.ctrl_pt_dist * 0.5),
                                                                        start_pt, local_target_pt))
      {
        vector<Eigen::Vector3d> rrt_path = bspline_optimizer_->informed_rrt_star_->getPath();
        if (rrt_path.size() < 2)
        {
          ROS_WARN("[PLAN] Informed RRT* returned too few points for initialization.");
          continous_failures_count_++;
          return false;
        }

        point_set.clear();
        point_set.push_back(rrt_path.front());
        const double sample_dist = std::max(0.15, pp_.ctrl_pt_dist);
        double carry = 0.0;
        for (size_t i = 1; i < rrt_path.size(); ++i)
        {
          Eigen::Vector3d p0 = rrt_path[i - 1];
          Eigen::Vector3d p1 = rrt_path[i];
          Eigen::Vector3d seg = p1 - p0;
          double len = seg.norm();
          if (len < 1e-6)
            continue;
          Eigen::Vector3d dir = seg / len;
          double traveled = sample_dist - carry;
          while (traveled < len)
          {
            point_set.push_back(p0 + dir * traveled);
            traveled += sample_dist;
          }
          carry = len - (traveled - sample_dist);
        }
        if ((point_set.back() - rrt_path.back()).norm() > 1e-3)
          point_set.push_back(rrt_path.back());

        while (point_set.size() < 7)
        {
          vector<Eigen::Vector3d> densified;
          densified.reserve(point_set.size() * 2);
          for (size_t i = 0; i + 1 < point_set.size(); ++i)
          {
            densified.push_back(point_set[i]);
            densified.push_back(0.5 * (point_set[i] + point_set[i + 1]));
          }
          densified.push_back(point_set.back());
          point_set.swap(densified);
        }
        ...
        if (segments.empty() && bspline_optimizer_->needFreshReplan())
        {
          ROS_WARN("[PLAN] RRT* initialization still overlaps obstacles; planning failed.");
          continous_failures_count_++;
          return false;
        }
      }
      ...
      {
        ROS_WARN("[PLAN] Informed RRT* failed to rebuild initialization.");
        continous_failures_count_++;
        return false;
      }
#else
    ...
    // FIX: Re-enable feasibility check in swarm mode. This is critical for safety.
    double ratio;
    bool flag_step_2_success = true;
    if (!pos.checkFeasibility(ratio, false))
    {
      cout << "Need to reallocate time, ratio=" << ratio << endl;
    ...
    if (!flag_step_2_success)
    {
      printf("\033[34m[PLAN] Refined trajectory still invalid. Planning failed.\033[0m\n");
      continous_failures_count_++;
      return false;
    }

    // FIX: Final collision validation before publishing.
    // This catches cases where the optimized B-spline deviates significantly
    // from the RRT* guide path and clips into obstacles.
    // Also checks 100% of the trajectory (not just 2/3).
    bool safe = bspline_optimizer_->validateFinalTrajectory(pos.getControlPoint(), ts, true);
    if (!safe)
    {
      ROS_ERROR("[PLAN] FINAL COLLISION DETECTED! Trajectory rejected. "
                 "Try increasing obstacles_inflation, reducing max_vel, "
