...
namespace ego_planner
{
  ...
  private:
    ...
    double local_horizon_min_;
    double emergency_time_;
    double goal_clearance_, goal_search_radius_;
    double goal_surround_check_radius_, goal_surround_hit_ratio_;
    double escape_trigger_clearance_, escape_target_clearance_, escape_search_radius_, escape_reached_thresh_;
    ...
    bool escape_target_active_;
    int escape_attempt_count_;
    int escape_max_attempts_;
    int escape_trigger_fail_times_;
    Eigen::Vector3d escape_target_pt_;
    Eigen::Vector3d actual_goal_pt_;
    bool have_actual_goal_;
    ...
    bool sanitizeGoalWaypoint(const Eigen::Vector3d &raw_wp, Eigen::Vector3d &safe_wp);
    double queryMapClearance(const Eigen::Vector3d &pos);
    ...
    bool selectEscapeTarget(const Eigen::Vector3d &start_pt, const Eigen::Vector3d &actual_target,
                            Eigen::Vector3d &escape_target);
    bool activateEscapeTarget(const Eigen::Vector3d &start_pt, const Eigen::Vector3d &actual_target);
    void clearEscapeTarget(const std::string &reason);
    bool tryEscapeFromStuck(const std::string &caller);
