...
#include <Eigen/Eigen>
// === PLANNER SELECTION ===
// Set this define to switch between A* and InformedRRT*.
// Comment out (#define USE_INFORMED_RRT_STAR) to use A*.
// Uncomment  (#define USE_INFORMED_RRT_STAR) to use InformedRRT*.
#define USE_INFORMED_RRT_STAR
#ifdef USE_INFORMED_RRT_STAR
#include <path_searching/informed_rrt_star.h>
#else
// A* support has been removed. Use InformedRRT* instead.
...
namespace ego_planner
{
  ...
  public:
...
#ifdef USE_INFORMED_RRT_STAR
    InformedRRTstar::Ptr informed_rrt_star_;
#else
    // DISABLED: Original A* member (see dyn_a_star.h)
    // AStar::Ptr a_star_;
#endif
    ...
    // Safety: validate final B-spline against inflated map before publishing
    // Returns true if safe, false if collision detected.
    // Also checks 100% of trajectory (not just 2/3).
    bool validateFinalTrajectory(const Eigen::MatrixXd &ctrl_pts, double ts, bool verbose = false);

    inline int getOrder(void) { return order_; }
    inline double getSwarmClearance(void) { return swarm_clearance_; }
    inline bool needFreshReplan(void) const { return force_stop_type_ == STOP_FOR_REBOUND || force_stop_type_ == STOP_FOR_ERROR; }
