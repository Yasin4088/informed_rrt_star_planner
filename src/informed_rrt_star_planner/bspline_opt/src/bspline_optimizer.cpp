...
namespace ego_planner
{
  namespace
  {
    bool pointHasClearance(const GridMap::Ptr &map, const Eigen::Vector3d &pos, double required_clearance)
    {
      if (map->getInflateOccupancy(pos))
        return false;

      const double resolution = map->getResolution();
      if (required_clearance <= resolution)
        return true;
      ...
      static const std::vector<Eigen::Vector3d> dirs = {
          ...
          Eigen::Vector3d(1, 1, 0),   Eigen::Vector3d(1, -1, 0),  Eigen::Vector3d(-1, 1, 0),
          Eigen::Vector3d(-1, -1, 0), Eigen::Vector3d(1, 0, 1),   Eigen::Vector3d(1, 0, -1),
          Eigen::Vector3d(-1, 0, 1),  Eigen::Vector3d(-1, 0, -1), Eigen::Vector3d(0, 1, 1),
          Eigen::Vector3d(0, 1, -1),  Eigen::Vector3d(0, -1, 1),  Eigen::Vector3d(0, -1, -1)};

      for (double radius = resolution; radius <= required_clearance; radius += resolution)
      {
        for (const auto &dir_raw : dirs)
        {
          Eigen::Vector3d dir = dir_raw.normalized();
          if (map->getInflateOccupancy(pos + dir * radius))
            return false;
        }
      }
      return true;
    ...
    bool findSafeIndex(const GridMap::Ptr &map, const Eigen::MatrixXd &points, int start_idx, int end_idx, int step,
                       double required_clearance, int &safe_idx)
    {
      for (int idx = start_idx; (step > 0) ? (idx <= end_idx) : (idx >= end_idx); idx += step)
      {
        if (pointHasClearance(map, points.col(idx), required_clearance))
        {
          safe_idx = idx;
          return true;
        }
    ...
    /** Integer index ranges on the B-spline lattice; merge overlaps for a valid “how many cp affected” count. */
    int overlappedCtrlUnionSpan(const vector<std::pair<int, int>> &segs)
    {
      if (segs.empty())
        return 0;
      vector<std::pair<int, int>> sorted = segs;
      sort(sorted.begin(), sorted.end(),
           [](const std::pair<int, int> &a, const std::pair<int, int> &b) { return a.first < b.first; });
      int lo = sorted[0].first;
      int hi = sorted[0].second;
      int sum = 0;
      for (size_t i = 1; i < sorted.size(); ++i)
      {
        if (sorted[i].first <= hi + 1)
          hi = std::max(hi, sorted[i].second);
        else
        {
          sum += std::max(0, hi - lo + 1);
          lo = sorted[i].first;
          hi = sorted[i].second;
        }
      }
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    constexpr double SAFE_ENDPOINT_EXTRA = 0.10;
    const double safe_endpoint_radius = dist0_ + SAFE_ENDPOINT_EXTRA;
    ...
    vector<std::pair<int, int>> safe_segment_ids;
    for (const auto &seg : segment_ids)
    {
      int safe_in = -1;
      int safe_out = -1;
      findSafeIndex(grid_map_, init_points, seg.first, order_, -1, safe_endpoint_radius, safe_in);
      findSafeIndex(grid_map_, init_points, seg.second, (int)init_points.cols() - order_ - 1, 1, safe_endpoint_radius, safe_out);

      if (safe_in < 0 || safe_out < 0 || safe_in >= safe_out)
      {
        ROS_WARN("[Bspline] Cannot find safe entry/exit control points for a collision segment. "
                 "Requesting fresh trajectory.");
        force_stop_type_ = STOP_FOR_REBOUND;
        vector<std::pair<int, int>> blank_ret;
        return blank_ret;
      }
      safe_segment_ids.push_back(std::pair<int, int>(safe_in, safe_out));
    }
    segment_ids.swap(safe_segment_ids);

    const int checked_ctrl_pts = std::max(1, i_end - order_ + 1);
    const int overlapped_union = overlappedCtrlUnionSpan(segment_ids);
    const double overlap_ratio =
        std::min(1.0, static_cast<double>(overlapped_union) / static_cast<double>(checked_ctrl_pts));
    for (const auto &seg : segment_ids)
    {
      const double segment_ratio = std::min(
          1.0,
          static_cast<double>(std::max(0, seg.second - seg.first + 1)) / static_cast<double>(checked_ctrl_pts));
      if (segment_ratio > 0.30 || overlap_ratio > 0.40)
      {
        ROS_WARN("[Bspline] Initial trajectory overlaps obstacles too much for local rebound "
                 "(segment_ratio=%.2f, overlap_ratio=%.2f). Requesting fresh trajectory.",
                 segment_ratio, overlap_ratio);
        force_stop_type_ = STOP_FOR_REBOUND;
        vector<std::pair<int, int>> blank_ret;
        return blank_ret;
      }
    ...
    /*** InformedRRT* search (drop-in replacement for A*) ***/
    vector<vector<Eigen::Vector3d>> guide_paths;
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      //cout << "in=" << in.transpose() << " out=" << out.transpose() << endl;
      Eigen::Vector3d in(init_points.col(segment_ids[i].first)), out(init_points.col(segment_ids[i].second));
      if (informed_rrt_star_->InformedRRTstarSearch(/*(in-out).norm()/10+0.05*/ 0.1, in, out))
      {
        guide_paths.push_back(informed_rrt_star_->getPath());
      }
      else
      {
        ROS_ERROR("InformedRRT* error, force return!");
        force_stop_type_ = STOP_FOR_REBOUND;
        vector<std::pair<int, int>> blank_ret;
        return blank_ret;
      }
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        int Astar_id = guide_paths[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
        double val = (guide_paths[i][Astar_id] - init_points.col(j)).dot(ctrl_pts_law), last_val = val;
        while (Astar_id >= 0 && Astar_id < (int)guide_paths[i].size())
        {
          ...
          val = (guide_paths[i][Astar_id] - init_points.col(j)).dot(ctrl_pts_law);

          if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
          {
            intersection_point =
                guide_paths[i][Astar_id] +
                ((guide_paths[i][Astar_id] - guide_paths[i][last_Astar_id]) *
                 (ctrl_pts_law.dot(init_points.col(j) - guide_paths[i][Astar_id]) / ctrl_pts_law.dot(guide_paths[i][Astar_id] - guide_paths[i][last_Astar_id])) // = t
                );
      ...
      if (segment_ids[i].second - segment_ids[i].first == 1)
      {
        ...
        int Astar_id = guide_paths[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
        double val = (guide_paths[i][Astar_id] - middle_point).dot(ctrl_pts_law), last_val = val;
        while (Astar_id >= 0 && Astar_id < (int)guide_paths[i].size())
        {
          last_Astar_id = Astar_id;
          ...
          val = (guide_paths[i][Astar_id] - middle_point).dot(ctrl_pts_law);
          ...
          if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
          {
            intersection_point =
                guide_paths[i][Astar_id] +
                ((guide_paths[i][Astar_id] - guide_paths[i][last_Astar_id]) *
                 (ctrl_pts_law.dot(middle_point - guide_paths[i][Astar_id]) / ctrl_pts_law.dot(guide_paths[i][Astar_id] - guide_paths[i][last_Astar_id])) // = t
                );
                                                 ...
                                                 Eigen::MatrixXd &gradient, int iter_num, double smoothness_cost)
  {
    ...
    force_stop_type_ = DONT_STOP;
    // FIX: Removed smoothness gate. Collision checking is critical safety and must run
    // regardless of how smooth the trajectory is. iter_num > 3 still guards against
    // checking too early when trajectory is still rapidly changing.
    if (iter_num > 3)
    {
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        bool found_in = findSafeIndex(grid_map_, cps_.points, i - 1, 0, -1, safe_endpoint_radius, in_id);
        if (!found_in) // fail to get a safe free point
        {
          ROS_WARN("[Bspline] Cannot find a safe entry point before obstacle overlap. Requesting full replan.");
          force_stop_type_ = STOP_FOR_REBOUND;
          return false;
        }

        bool found_out = findSafeIndex(grid_map_, cps_.points, i + 1, cps_.size - 1, 1, safe_endpoint_radius, out_id);
        if (!found_out) // fail to get a safe free point
        {
    ...
    {
      const int checked_ctrl_pts = std::max(1, i_end - (order_ - 1) + 1);
      const int overlapped_union = overlappedCtrlUnionSpan(segment_ids);
      const double overlap_ratio =
          std::min(1.0, static_cast<double>(overlapped_union) / static_cast<double>(checked_ctrl_pts));
      for (const auto &seg : segment_ids)
      {
        const double segment_ratio = std::min(
            1.0,
            static_cast<double>(std::max(0, seg.second - seg.first + 1)) /
                static_cast<double>(checked_ctrl_pts));
        if (segment_ratio > 0.30 || overlap_ratio > 0.40)
        {
          ROS_WARN("[Bspline] Current trajectory deeply overlaps obstacles "
                   "(segment_ratio=%.2f, overlap_ratio=%.2f). Rejecting local rebound.",
                   segment_ratio, overlap_ratio);
          force_stop_type_ = STOP_FOR_REBOUND;
          return false;
        }
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
          ...
          int Astar_id = guide_paths[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
          double val = (guide_paths[i][Astar_id] - cps_.points.col(j)).dot(ctrl_pts_law), last_val = val;
          while (Astar_id >= 0 && Astar_id < (int)guide_paths[i].size())
          {
            last_Astar_id = Astar_id;
            ...
            val = (guide_paths[i][Astar_id] - cps_.points.col(j)).dot(ctrl_pts_law);
            ...
            if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
            {
              intersection_point =
                  guide_paths[i][Astar_id] +
                  ((guide_paths[i][Astar_id] - guide_paths[i][last_Astar_id]) *
                   (ctrl_pts_law.dot(cps_.points.col(j) - guide_paths[i][Astar_id]) / ctrl_pts_law.dot(guide_paths[i][Astar_id] - guide_paths[i][last_Astar_id])) // = t
                  );
...
// ============================================================
// Safety: validate final B-spline before publishing
// Checks 100% of trajectory against inflated map (not just 2/3)
// Also checks velocity/acceleration feasibility
// Returns false (reject trajectory) if any collision or severe infeasibility found
// ============================================================

bool BsplineOptimizer::validateFinalTrajectory(const Eigen::MatrixXd &ctrl_pts, double ts, bool verbose)
{
  UniformBspline traj(ctrl_pts, order_, ts);

  double tm, tmp;
  traj.getTimeSpan(tm, tmp);
  double dur = tmp - tm;

  // Sample at map resolution: at least 1 check per grid cell
  double t_step = ts / 2.0;  // Sample every half knot interval for safety
  t_step = max(t_step, grid_map_->getResolution() / (max_vel_ + 1e-6));

  int collision_count = 0;
  int infeasible_count = 0;
  Eigen::Vector3d first_collision_pt;
  double first_collision_t = -1;

  double max_vel_seen = 0, max_acc_seen = 0;

  // Evaluate velocity and acceleration limits from control points
  double enlarged_vel_lim = max_vel_ * (1.0 + 0.05) + 1e-4;
  double enlarged_acc_lim = max_acc_ * (1.0 + 0.05) + 1e-4;

  for (double t = tm; t <= tmp + 1e-4; t += t_step)
  {
    ...
    {
      collision_count++;
      if (first_collision_t < 0)
      {
        first_collision_pt = pos;
        first_collision_t = t;
      }
    }
  }

  // Check velocity/acceleration feasibility from control points
  const Eigen::MatrixXd &P = ctrl_pts;
  for (int i = 0; i < P.cols() - 1; ++i)
  {
    Eigen::Vector3d vel = order_ * (P.col(i + 1) - P.col(i)) / ts;
    for (int d = 0; d < 3; ++d)
    {
      if (fabs(vel(d)) > enlarged_vel_lim)
        infeasible_count++;
      max_vel_seen = max(max_vel_seen, fabs(vel(d)));
    }
  }
  for (int i = 0; i < P.cols() - 2; ++i)
  {
    Eigen::Vector3d acc = order_ * (order_ - 1) *
                           ((P.col(i + 2) - P.col(i + 1)) / ts - (P.col(i + 1) - P.col(i)) / ts) / ts;
    for (int d = 0; d < 3; ++d)
    {
      if (fabs(acc(d)) > enlarged_acc_lim)
        infeasible_count++;
      max_acc_seen = max(max_acc_seen, fabs(acc(d)));
    }
  }

  if (verbose || collision_count > 0 || infeasible_count > 0)
  {
    ROS_WARN("[validateFinalTraj] collisions=%d, infeasible=%d, max_vel=%.2f/%.2f, max_acc=%.2f/%.2f",
             collision_count, infeasible_count,
             max_vel_seen, max_vel_,
             max_acc_seen, max_acc_);
    if (collision_count > 0)
      ROS_WARN("[validateFinalTraj] FIRST COLLISION at t=%.2f, pos=(%.2f,%.2f,%.2f)",
               first_collision_t,
               first_collision_pt(0), first_collision_pt(1), first_collision_pt(2));
  }

  // Reject: any collision point found
  if (collision_count > 0)
    return false;

  // Allow: minor infeasibility, but warn
  if (infeasible_count > 0)
    ROS_WARN("[validateFinalTraj] Trajectory slightly exceeds limits but allowed (infeasible_count=%d). Consider reducing max_vel/max_acc.", infeasible_count);
